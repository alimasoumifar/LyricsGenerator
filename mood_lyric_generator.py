...
def calculate_probability(cur_word, previous):
    ...
    trigram_prob = (Data.trigramCounts[second_prev_word][prev_word][cur_word] + 1) / (
                Data.bigramCounts[second_prev_word][prev_word] + len(Data.vocab.keys()))
...
def calculate_probability_model(cur_word, previous, bigramModel, trigramModel, vocab):
    ...
    trigram_prob = (trigramModel[second_prev_word][prev_word][cur_word] + 1) / (
                bigramModel[second_prev_word][prev_word] + len(vocab.keys()))
...
def train_model():
    ...
    for adj in adjective_lyrics:
        ...
        probabilities = calculate_all_probability_model(adjective_lyrics[adj], new_lang_model[1], new_lang_model[2],
                                                        new_lang_model[0])
...
def main():
    ...
    print("p(wi=”special” | wi - 2 =”something ”, wi - 1 =”very”)) = " + str(
        calculate_probability("special", ["something", "very"])))
    try:
        print("p(wi=”funny” | wi - 2 =”something ”, wi - 1 =”very”) = " + str(
            calculate_probability("funny", ["something", "very"])))
    except KeyError:
